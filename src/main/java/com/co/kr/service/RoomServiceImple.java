...
public class RoomServiceImple implements RoomService {
	...
	public int rmfileProcess(FileListVO fileListVO, MultipartHttpServletRequest request, HttpServletRequest httpReq) {
		...
		// content domain 생성
		RoomContentDomain roomContentDomain = RoomContentDomain.builder().mbId(session.getAttribute("id").toString())
				.rmTitle(fileListVO.getTitle()).rmContent(fileListVO.getContent()).rmType(fileListVO.getType())
				.rmMemo(fileListVO.getMemo()).build();

		if (fileListVO.getIsEdit() != null) {
			roomContentDomain.setRmSeq(Integer.parseInt(fileListVO.getSeq()));
			System.out.println("수정업데이트");
			// db 업데이트
			roomMapper.rmContentUpdate(roomContentDomain);
		} else {
			// db 인서트
			roomMapper.rmcontentUpload(roomContentDomain);
			System.out.println(" db 인서트");

		}

		// file 데이터 db 저장시 쓰일 값 추출
		int rmSeq = roomContentDomain.getRmSeq();
		String mbId = roomContentDomain.getMbId();
		...
		if (fileListVO.getIsEdit() != null) { // 수정시
			...
			List<RoomFileDomain> fileList = null;
			...
			for (MultipartFile multipartFile : multipartFiles) {
				...
				if (!multipartFile.isEmpty()) { // 수정시 새로 파일 첨부될때 세션에 담긴 파일 지우기
					...
					if (session.getAttribute("files") != null) {
						...
						fileList = (List<RoomFileDomain>) session.getAttribute("files");
		...
		// 저장 root 경로만들기
		Path rootPath = Paths.get(new File("C://").toString(), "roomfiles", File.separator).toAbsolutePath().normalize();
		...
		for (MultipartFile multipartFile : multipartFiles) {
			...
			if (!multipartFile.isEmpty()) { // 파일 있을때
				...
				try {
					...
					// 파일 domain 생성
					RoomFileDomain roomFileDomain = RoomFileDomain.builder().rmSeq(rmSeq).mbId(mbId)
							.upOriginalFileName(origFilename).upNewFileName("resources/roomfiles/" + newFileName) // WebConfig에
																													// 동적
																													// 이미지
																													// 폴더
																													// 생성
					...
					System.out.println(roomFileDomain);
					// db 인서트
					roomMapper.rmfileUpload(roomFileDomain);
